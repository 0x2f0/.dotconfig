		...
		actions = {
			["default"] = function(selected, _opts)
				if selected and selected[1] then 
					local page_name = vim.split(selected[1], "%s+")[1]
					vim.cmd("vertical Man " .. vim.fn.fnameescape(page_name))
				end
			end
		}
	})
...
vim.keymap.set({"n"}, "<leader>sd", function()  
	local configDir = "~/.config/.dotconfig"
	picker.files({ cwd = configDir })
end, 
{ desc = "Search the dotfiles and open in tab" })
